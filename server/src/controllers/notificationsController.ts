...
import { INotification, Notification, NotificationType } from "../models/Notifications";
...
export const getNotificationsByEmail = async (req: Request, res: Response): Promise<void> => {
  ...
  export const updateNotificationReadState = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      res.status(200).json({ message: "Notification state updated", notification });
    } catch (error) {
