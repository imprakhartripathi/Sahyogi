...
const aiController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const prioritizedTasks: IPrioritizedTask[] = user.tasks
      ...
      .map((task) => {
        ...
        // Normalize complexity (assuming max complexity is 100)
        const normalizedComplexity = taskObj.taskComplexityPoint / 100;

        // Calculate days until deadline (or 0 if no deadline)
        const daysUntilDeadline = taskObj.dateDeadline
          ? (taskObj.dateDeadline.getTime() - currentDate.getTime()) /
            (1000 * 60 * 60 * 24)
          : 0;
